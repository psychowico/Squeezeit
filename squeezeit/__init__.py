...
from squeezeit import slimmer, jsmin
import sys
import os
import re
import hashlib
...
def writedata(configfile, config, output, filename, rawdata, binary_mode=False):
    ...
    try:
        f = open(outfile, 'wb' if binary_mode else 'w')
        f.write(rawdata)
        f.close()
    except:
